...
public class AdvancedSearchingbyISBN {
	...
	By RL_LINK_RECORD_LINKS = By
			.xpath(".//*[contains(text(),'ISBN-10')]/following-sibling::dd");

	// Test data
	String userName = "SelAdmin";
	String password = "12345678";
	String isbnumber = "0-674-03612-3";
	String bipnumoption = "ISBN/UPC/EAN";
	...
	public void loginwithComplexaction() {
		...
		for(int i=0; i<sizeofbipnumber; i++){
            ...
            matchedcounter = (str1.indexOf(isbnumber)>=0 ? matchedcounter + 1 : matchedcounter);
			}
		
		 System.out.println("total find "+matchedcounter+" kind of book with"+ isbnumber);
	...
	private void fillCondtioninAdvancedpage(WebDriver driver) {
		...
		clearSendKeys(inputfram, isbnumber);
		//select option 
		Select dropdown = new Select(driver.findElement(ADVANCED_SELECTOPTION1));
		dropdown.selectByVisibleText(bipnumoption);
		// click search button
